...
> 依据 [`constitution.md`](constitution.md) 第 11 条「每次使用 Agent 必须记录」要求，逐次记录 Claude Code 的辅助过程，包含输入 Prompt、修改文件、输出摘要、人工审查与测试结果，保证 Spec-Driven 开发全过程可追溯。

- **工具**：Claude Code
- **总次数**：9 次
- **覆盖阶段**：需求分析 → UML 建模 → 架构/详细设计 → 代码实现 → 测试 → 前端 → Specs 审查修正
- **说明**：第 1–7 次的「Git 提交」为当时增量提交记录（部分为描述性信息或旧哈希）；项目最终整理为 8 个正式提交，完整哈希见 [`specs/21-git-history.md`](21-git-history.md)。
...
- **使用工具**：Claude Code
- **使用阶段**：前端样式统一
- **使用任务**：将系统中所有输入框（文本/邮箱/电话/数字/搜索）样式统一为与下拉选择框一致，并修正系统管理员「人员管理」「图书与馆藏」等处不合理的尺寸与排版。
- **输入 Prompt 摘要**：把读者注册、图书管理员借还书、系统管理员图书与馆藏、人员管理等表单的填写框样式改为与系统下拉选择框一致、尺寸排版合理；随后推广到项目全部输入框。
- **Agent 修改文件**：static/css/app.css、static/js/app.js
- **输出摘要**：输入框与 select 共享统一控件样式（appearance:none 去除原生外观、44px 高、8px 圆角、同边框/悬浮/聚焦）；人员管理姓名输入框限制 320px；馆藏副本改为「添加/移除」两行布局并限制宽度。
- **人工审查结果**：前端改动不影响后端；经 node --check 校验通过，待人工在浏览器最终确认。
- **测试结果**：35 个后端测试全部通过（前端改动不影响测试）。
- **Git 提交**：ebc6242（前端修改2）

## 第 9 次使用

- **使用工具**：Claude Code
- **使用阶段**：Specs 审查与修正 + AI 使用记录补全
- **使用任务**：对照代码逐份核查 specs 目录 21 份编号文档（00–20）与 7 张 PlantUML 图，修正错误与不足，并补全本记录。
- **输入 Prompt 摘要**：再次检查 specs 文件夹中的文件和 puml 图片，修正错误或不足；同时完善 AI 使用说明。
- **Agent 修改文件**：specs/14-api-spec.md、specs/06-domain-class-diagram.puml、specs/04-use-case-model.puml、specs/20-design-class-diagram.puml、specs/05-domain-model.md、specs/02-requirements.md、specs/03-use-cases.md、specs/19-ai-usage-log.md
- **输出摘要**：①API 规范补全缺失端点（读者/借阅证/规则/人员列表与人员增删）并纠正请求示例（card_no/barcode）与失败分支（补 404 非本馆藏书）；②领域类图补 5 个状态枚举（CardStatus/ItemStatus/LoanStatus/ReservationStatus/FineStatus）；③用例图补「删除图书标题」「删除馆藏副本」；④设计类图修正 AdminService 依赖并补全 DTO；⑤FR/UC 补系统管理员维护、规则维护改 upsert；⑥补全本记录第 8、9 次使用。
- **人工审查结果**：待人工审查。
- **测试结果**：35 个测试全部通过（文档改动不影响代码）。
- **Git 提交**：2f0e01d（对specs进行优化）

## 使用总结

- **共 9 次** Claude Code 辅助，产出：22 份编号文档（00–21）+ 7 张 PlantUML 图 + 34 个后端源文件（`app/`）+ 35 个自动化测试。
- **Spec-Driven 全程贯彻**：baseline specs 冻结后，实现阶段不改 specs（见 [`16-tasks.md`](16-tasks.md) 的「禁止修改」列与 [`constitution.md`](constitution.md)）。
- **测试递增**：25 → 29 → 33 → 35，每轮改动均跑通全部测试（见各次「测试结果」）。
- **合规要点**：分层架构、策略/仓储/服务层/DTO 模式、角色权限隔离、借阅证号格式（BR-002）、规则非负校验等均已落实并通过测试验证。
- **人工审查**：每轮输出均经人工核对后方可进入下一阶段（见各次「人工审查结果」）。
